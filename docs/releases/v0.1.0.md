...
`BASIS_CONSOLE_MODE` (default `operator`). This is a **UX/copy concern only** and
changes no runtime behavior, architectural boundary, API, or capability; the run
modes above are orthogonal to it and sample/live/future labels stay honest in
both.
...
- **`operator` (default)** — professional, concise, operator-focused. Minimal
  ...
  screenshots. Large pedagogical explainer panels are hidden; short boundary
  statements, the footer boundary note, and sample/live/future labels remain.
- **`training`** — educational. Adds a visible top-level training banner, a
  per-page "What this page teaches" callout, and a standard BASIS architecture
  explanation (console observes/inspects/submits/explains, gateway enforces, core
  evaluates, adapters normalize, identity is the future `basis-identity` service,
  deploy/demo/topology are future layers). Explicitly **not** an operator view.

The mode names the *audience* of the interface, not a deployment environment.
Both modes present the **same application** — same pages, navigation, controls,
routing, and workflows; training mode only **adds** educational content and never
changes layout or functionality. An invalid `BASIS_CONSOLE_MODE` fails
configuration validation at startup with a clear, actionable message.
